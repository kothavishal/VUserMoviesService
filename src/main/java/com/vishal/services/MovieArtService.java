...
public class MovieArtService {
	...
	public Observable<String> movieImage(Integer movieId) {
		...
		return Observable.<String> create(obsEmitter -> {
			try {
				...
				if (result == null) {
					...
					if (result == null) {
						obsEmitter.onError(new UserMoviesException("Movie Image not found for movie id " + movieId,
				...
				logger.error(
						...
						e);
				obsEmitter.onError(new UserMoviesException("Application Error. Please try refreshing.",
